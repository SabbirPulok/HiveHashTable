...
number_of_ops = [20, 21, 22, 23, 24, 25, 26, 27] #2^n
...
def plot_radar_chart(data, factors, fig_name, fig_title, rmax=100.0):
    ...
    plt.title(fig_title, fontsize=16, fontweight='bold')
    ax.legend(loc='lower right', bbox_to_anchor=(1.1, 1.1), fontsize=12, frameon=False)
...
def main ():
    ...
    for load_factor in load_factors:
        for ops in number_of_ops:
            for layout in data_layouts:
                ...
                # collect NCU metrics and merge into the main metrics dict so we produce one combined CSV row
                ncu_output = run_ncu(params, BENCHMARK_EXECUTABLE, "hive_mixed_kernel")
                ncu_metrics = ncu_parse_output(ncu_output)
                for k, v in ncu_metrics.items():
                    metrics[k] = v
                results_concurrent_mix.append(metrics)
                ...
                # for k, v in ncu_metrics.items():
                #     metrics[k] = v
                # results_inserts_only.append(metrics)
    ...
    #---------- Concurrent Mix Experiment-------------
    print("Writing Concurrent Mix Results and Generating Plots...")
    filename = f"data_layout_experiment_concurrent_mix_{timestamp}.csv"
    w_csv(results_concurrent_mix, RESULTS_DIR, filename, all_possible_metric_keys)
    #read csv
    # results_concurrent_mix = r_csv(RESULTS_DIR, filename)
    x, y = extract_xy_single_map("load_factor", target_lf, results_concurrent_mix, 'total_ops', 'throughput_mops')

    fig_name = f"data_layout_experiment_concurrent_mix_throughput_{timestamp}.png"
    fig_title = f"Data layout Experiment - Concurrent Mix Throughput (lf={target_lf})"
    ...
    #Memory Behavior (Memory Throughput, l1_cache_hit_rate, l2_cache_hit_rate, l1_cache_throughput, l2_cache_throuput)

    fig_name = f"data_layout_experiment_concurrent_mix_memory_behavior_{timestamp}.png"
    fig_title = f"Data layout Experiment - Concurrent Mix Operations Memory Behavior (N={target_table_size_in_millions} millions)"
    x_label = "Number of Operations (millions)"
    y_label = "Memory Throughput (GB/s)"

    target_table_size = 1 << number_of_ops[-1]
    memory_behavior_data_mix_ops = filter_data("table_size", target_table_size, results_concurrent_mix, 'data_layout', memory_behavior_cols)
    plot_radar_chart(memory_behavior_data_mix_ops, memory_behavior_cols, fig_name, fig_title, rmax=100.0)


    # # ------------------Inserts Only------------------------
    # print("Writing Inserts Only Results and Generating Plots...")
    # filename = f"data_layout_experiment_inserts_only_{timestamp}.csv"
    # w_csv(results_inserts_only, RESULTS_DIR, filename, all_possible_metric_keys)

    # x, y = extract_xy_single_map("load_factor", target_lf, results_inserts_only, 'total_ops', 'throughput_mops')
    # fig_name = f"data_layout_experiment_inserts_only_throughput_{timestamp}.png"
    # fig_title = f"Data layout Experiment - Inserts Only Throughput (lf={target_lf})"
    # x_label = "Number of Operations (millions)"
    # y_label = "Throughput (Mops/s)"

    # plot_multi_bar_chart(x, y, data_layouts, fig_name, fig_title, x_label, y_label)

    # fig_name = f"data_layout_experiment_inserts_only_memory_behavior_{timestamp}.png"
    # fig_title = f"Data layout Experiment - Inserts Only Memory Behavior (number of inserts = {target_table_size_in_millions} millions)"
    # memory_behavior_data_inserts = filter_data("table_size", target_table_size, results_inserts_only, 'data_layout', memory_behavior_cols)
    # plot_radar_chart(memory_behavior_data_inserts, memory_behavior_cols, fig_name, fig_title, rmax=100.0)
