...
__device__ __forceinline__ void lock_bucket(HiveHashTable* table, const uint32_t bucket_idx)
{
    ...
    ))
    {
        ...
        //Could be add optional backoff here
    }
}
...
)
{
    ...
    ulonglong2 two_kvs = bucket_vec[lane_id];

    // Check both keys for a match
    match_a = (unpackKey(two_kvs.x) == key);
    match = match_a || (unpackKey(two_kvs.y) == key);
    ...
    if (lane_id == winner)
    {
        ...
        cached_kv = match_a ? two_kvs.x : two_kvs.y;
        ...
        success = atomic_slot.compare_exchange_strong(
            expected,
            desired,
...
)
{
    ...
    if (lane_id == 0)
    {
        //free_mask = __ldcv(&table->freeMask[bucket_idx]); //invalidate L1 cache
...
)
{
    ...
    {
        uint32_t b_idx = HashPolicy::get_bucket(i, key, table->num_buckets);
        if(claim_and_commit_slot(table, b_idx, kv, active, tile) != -1) 
        {
            #if BREAKDOWN_INSERT
            end_time = clock64();
