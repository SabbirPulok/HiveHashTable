...
void build_search_bulk_experiment(uint32_t num_keys_start,
  ...
  for (uint32_t num_keys = num_keys_start; num_keys <= num_keys_end;
    ...
    const uint32_t expected_elements_per_bucket =
        (expected_chain * 9) * num_elements_per_unit; // load factor 0.9
    const uint32_t num_buckets = (num_keys + expected_elements_per_bucket - 1) /
                                 expected_elements_per_bucket;
...
/*
 * In this experiment, we measure the time to "rehash" the table by creating a new
 * table with more buckets and inserting the same data.
 */
template <typename KeyT, typename ValueT>
void rehash_experiment(uint32_t num_keys,
                       uint32_t added_buckets,
                       std::string filename,
                       uint32_t device_idx = 0,
                       bool verbose = false) {
  rapidjson::Document doc;
  ...
  KeyT* h_key = new KeyT[num_keys];
  ValueT* h_value = new ValueT[num_keys];

  // === generating random key-values
  BatchedDataGen key_gen(num_keys, num_keys);
  key_gen.generate_random_keys(std::time(nullptr), /*num_msb = */ 0, true);
  auto f = [](uint32_t key) { return ~key; };

  auto buffer_ptr = key_gen.getSingleBatchPointer(num_keys, 0, 0);
  for (int i = 0; i < num_keys; i++) {
    h_key[i] = buffer_ptr[i];
    h_value[i] = f(h_key[i]);
  }

  // Target 90% load factor for the "old" table
  float load_factor = 0.9f;
  const uint32_t num_elements_per_unit = 15;
  uint32_t expected_elements_per_bucket = load_factor * num_elements_per_unit;
  uint32_t num_buckets_old = (num_keys + expected_elements_per_bucket - 1) / expected_elements_per_bucket;
  uint32_t num_buckets_new = num_buckets_old + added_buckets;

  if (verbose) {
    printf("Rehash Experiment:\n");
    printf("  Num Keys: %u\n", num_keys);
    printf("  Old Buckets: %u (Load Factor: %.2f)\n", num_buckets_old, load_factor);
    printf("  New Buckets: %u (Added: %u)\n", num_buckets_new, added_buckets);
  }

  // 1. Build Old Table (Warmup/Baseline)
  float old_build_time = 0.0f;
  float rehash_time = 0.0f;
  
  {
      gpu_hash_table<KeyT, ValueT, SlabHashTypeT::ConcurrentMap> hash_table_old(
          num_keys, num_buckets_old, device_idx, time(nullptr));
      
      old_build_time = hash_table_old.hash_build(h_key, h_value, num_keys);
      cudaDeviceSynchronize();

      // 2. Build New Table (Rehash)
      // Note: hash_table_old is still alive, so hash_table_new will use new memory.
      gpu_hash_table<KeyT, ValueT, SlabHashTypeT::ConcurrentMap> hash_table_new(
          num_keys, num_buckets_new, device_idx, time(nullptr));
      
      rehash_time = hash_table_new.hash_build(h_key, h_value, num_keys);
  } // Both destructors called here

  if (verbose) {
    printf("  Old Build Time: %.2f ms\n", old_build_time);
    printf("  Rehash (New Build) Time: %.2f ms\n", rehash_time);
  }

  rapidjson::Value object(rapidjson::kObjectType);
  object.AddMember("num_keys", rapidjson::Value().SetInt(num_keys), doc.GetAllocator());
  object.AddMember("num_buckets_old", rapidjson::Value().SetInt(num_buckets_old), doc.GetAllocator());
  object.AddMember("num_buckets_new", rapidjson::Value().SetInt(num_buckets_new), doc.GetAllocator());
  object.AddMember("added_buckets", rapidjson::Value().SetInt(added_buckets), doc.GetAllocator());
  object.AddMember("old_build_time_ms", rapidjson::Value().SetDouble(old_build_time), doc.GetAllocator());
  object.AddMember("rehash_time_ms", rapidjson::Value().SetDouble(rehash_time), doc.GetAllocator());
...
/*
 * In this experiment, we measure the search throughput for varying existence ratios.
 * We use the concurrent interface (batched_operations) to perform searches.
 */
template <typename KeyT, typename ValueT>
void query_experiment(uint32_t num_keys,
                      float search_exist_ratio,
                      float search_non_exist_ratio,
                      std::string filename,
                      uint32_t device_idx = 0,
                      bool verbose = false) {
  rapidjson::Document doc;
  ...
  // Use a fixed batch size for simplicity
  const uint32_t batch_size = 1 << 20; 
  const uint32_t num_initial_batches = (num_keys + batch_size - 1) / batch_size;
  const uint32_t total_num_keys = num_initial_batches * batch_size;
  
  const uint32_t search_batch_size = batch_size;

  float expected_chain = 0.9f; 
  const uint32_t num_elements_per_unit = 15;
  uint32_t expected_elements_per_bucket = (expected_chain * 3) * num_elements_per_unit;
  uint32_t num_buckets = (total_num_keys + expected_elements_per_bucket - 1) /
                           expected_elements_per_bucket;

  uint32_t* h_result = new uint32_t[total_num_keys + search_batch_size];

  gpu_hash_table<KeyT, ValueT, SlabHashTypeT::ConcurrentMap> hash_table(
          total_num_keys + search_batch_size, num_buckets, device_idx, time(nullptr));

  // Enough references for both initial build and search
  BatchedDataGen key_gen(total_num_keys + search_batch_size, batch_size);
  key_gen.generate_random_keys(std::time(nullptr), /*num_msb = */ 2,
                                   /*unique = */ true);

  float init_build_time = 0.0f;
  for (uint32_t batch_id = 0; batch_id < num_initial_batches; batch_id++) {
    key_gen.next_batch(1.0f, 0.0f, 0.0f); // Pure insertion
    init_build_time += hash_table.batched_operations(
        key_gen.h_batch_buffer_, h_result, batch_size, batch_id);
  }
  double init_load_factor = hash_table.measureLoadFactor();

  // Perform search with specified existence ratio
  // BatchedDataGen::next_batch(a_insert, b_delete, c_search_exist)
  // Search non-exist is calculated as 1.0 - (a + b + c)
  // We want c = search_exist_ratio, and we need to ensure search_non_exist_ratio
  // is consistent with what BatchedDataGen expects if it sums to 1.0.
  
  float concurrent_time = 0.0f;
  // We pass search_exist_ratio. BatchedDataGen will use 1.0 - search_exist_ratio for non-exist
  // if we pass 0 for insert and delete.
  key_gen.next_batch(0.0f, 0.0f, search_exist_ratio); 
  
  concurrent_time = hash_table.batched_operations(
            key_gen.h_batch_buffer_, h_result, search_batch_size, num_initial_batches);

  double final_load_factor = hash_table.measureLoadFactor();

  if (verbose) {
    printf("Query Experiment: exist_ratio = %.2f, non_exist_ratio = %.2f\n", 
           search_exist_ratio, search_non_exist_ratio);
    printf("  Search Rate: %.3f M op/s\n", float(search_batch_size) / concurrent_time / 1000.0f);
  }

  // Store results:
  rapidjson::Value object(rapidjson::kObjectType);
  object.AddMember("init_load_factor", rapidjson::Value().SetDouble(init_load_factor), doc.GetAllocator());
  object.AddMember("final_load_factor", rapidjson::Value().SetDouble(final_load_factor), doc.GetAllocator());
  object.AddMember("num_buckets", rapidjson::Value().SetInt(num_buckets), doc.GetAllocator());
  object.AddMember("initial_rate_mps", rapidjson::Value().SetDouble(double(total_num_keys) / init_build_time / 1000.0), doc.GetAllocator());
  object.AddMember("search_exist_ratio", rapidjson::Value().SetDouble(search_exist_ratio), doc.GetAllocator());
  object.AddMember("search_non_exist_ratio", rapidjson::Value().SetDouble(1.0f - search_exist_ratio), doc.GetAllocator());
  object.AddMember("concurrent_rate_mps", rapidjson::Value().SetDouble(double(search_batch_size) / concurrent_time / 1000.0), doc.GetAllocator());
  object.AddMember("num_keys", rapidjson::Value().SetInt(total_num_keys), doc.GetAllocator());
  object.AddMember("batch_size", rapidjson::Value().SetInt(search_batch_size), doc.GetAllocator());
  object.AddMember("num_init_batches", rapidjson::Value().SetInt(num_initial_batches), doc.GetAllocator());
  object.AddMember("num_batches", rapidjson::Value().SetInt(num_initial_batches + 1), doc.GetAllocator());
  object.AddMember("insert_ratio", rapidjson::Value().SetDouble(0.0f), doc.GetAllocator());
  object.AddMember("delete_ratio", rapidjson::Value().SetDouble(0.0f), doc.GetAllocator());
...
/*
 * In this experiment, we measure the time to "merge" buckets.
 * We start with a large number of buckets (low load factor, < 0.25) 
 * and rebuild into a smaller table (high load factor, ~0.9).
 */
template <typename KeyT, typename ValueT>
void merge_experiment(uint32_t num_keys,
                      uint32_t removed_buckets,
                      std::string filename,
                      uint32_t device_idx = 0,
                      bool verbose = false) {
  rapidjson::Document doc;
  ...
  // Target 90% load factor for the "new" (merged) table
  float target_load_factor = 0.9f;
  const uint32_t num_elements_per_unit = 15;
  uint32_t expected_elements_per_bucket = target_load_factor * num_elements_per_unit;
  uint32_t num_buckets_new = (num_keys + expected_elements_per_bucket - 1) / expected_elements_per_bucket;
  
  // The "old" table has 'removed_buckets' MORE buckets than the new one.
  uint32_t num_buckets_old = num_buckets_new + removed_buckets;

  if (verbose) {
    printf("Merge Experiment:\n");
    printf("  Num Keys: %u\n", num_keys);
    printf("  Old Buckets (Start): %u\n", num_buckets_old);
    printf("  New Buckets (Target): %u\n", num_buckets_new);
    printf("  Removed Buckets: %u\n", removed_buckets);
  }

  // 1. Build Old Table (Low Load Factor)
  float old_build_time = 0.0f;
  ...
  
  {
      ...
      if (verbose) {
        double lf = hash_table_old.measureLoadFactor();
        printf("  Old Table Built. Load Factor: %.4f (Should be < 0.25)\n", lf);
        printf("  Old Build Kernel Time: %.2f ms\n", old_build_time);
      }

      cudaDeviceSynchronize();

      // 2. Build New Table (Merge/Shrink)
      // Note: hash_table_old is still alive.
      if (verbose) printf("Building New Table (Merge)...\n");
      gpu_hash_table<KeyT, ValueT, SlabHashTypeT::ConcurrentMap> hash_table_new(
          num_keys, num_buckets_new, device_idx, time(nullptr));
      
      merge_time = hash_table_new.hash_build(h_key, h_value, num_keys);
      
      if (verbose) {
          printf("  Merge Kernel Time: %.2f ms\n", merge_time);
          double lf = hash_table_new.measureLoadFactor();
          printf("  New Table Load Factor: %.4f\n", lf);
      }
  } 
  ...
  object.AddMember("num_buckets_old", rapidjson::Value().SetInt(num_buckets_old), doc.GetAllocator());
  object.AddMember("num_buckets_new", rapidjson::Value().SetInt(num_buckets_new), doc.GetAllocator());
  object.AddMember("removed_buckets", rapidjson::Value().SetInt(removed_buckets), doc.GetAllocator());
  object.AddMember("old_build_time_ms", rapidjson::Value().SetDouble(old_build_time), doc.GetAllocator());
  object.AddMember("merge_time_ms", rapidjson::Value().SetDouble(merge_time), doc.GetAllocator());
