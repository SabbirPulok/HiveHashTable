...
int main(int argc, char** argv) {
  ...
  // mode 1 parameters:
  float lf_bulk_step = 1.0f;
  uint32_t lf_bulk_num_sample = 10;

  // mode 3 parameters:
  int num_batch = 2;
  int init_batch = 1;
  float insert_ratio = 0.5f;
  float delete_ratio = 0.1f;
  float search_exist_ratio = 0.4f;
  float search_non_exist_ratio = 0.0f;
  float lf_conc_step = 1.0f;
  int lf_conc_num_sample = 20;

  // mode 4 parameters:
  uint32_t added_buckets = 10000;
  ...
  if (cmdOptionExists(argv, argc + argv, "-search_non_exist_ratio"))
    search_non_exist_ratio =
        atof(getCmdOption(argv, argv + argc, "-search_non_exist_ratio"));
  ...
  if (cmdOptionExists(argv, argc + argv, "-added_buckets"))
    added_buckets = atoi(getCmdOption(argv, argv + argc, "-added_buckets"));
  ...
  switch (mode) {
    ...
    case 3:  // concurrent experiment:
      concurrent_batched_op_load_factor_experiment<KeyT, ValueT>(
          ...
          verbose);
      break;
    case 4: // rehash experiment
      rehash_experiment<KeyT, ValueT>(num_keys, added_buckets, filename, device_idx, verbose);
      break;
    case 5: // merge experiment
      merge_experiment<KeyT, ValueT>(num_keys, added_buckets, filename, device_idx, verbose);
      break;
    case 6: // query experiment
      query_experiment<KeyT, ValueT>(num_keys, search_exist_ratio, search_non_exist_ratio, filename, device_idx, verbose);
      break;
    default:
