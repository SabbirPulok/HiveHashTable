...
void bench_bcht(std::vector<key_type>& keys,
  ...
  std::string fname = "bcht_rates_lfeq" + std::to_string(int(load_factor * 100)) + ".csv";

  bool output_file_exist = std::filesystem::exists(dir + fname);
  std::fstream output(dir + fname, std::ios::app);

  std::vector<float> exist_ratios = {1.0f, 0.75f, 0.5f, 0.25f, 0.0f};
...
void bench_iht(std::vector<key_type>& keys,
  ...
  std::string fname = "iht_rates_lfeq" + std::to_string(int(load_factor * 100)) + ".csv";
...
void bench_p2bht(std::vector<key_type>& keys,
  ...
  std::string fname =
      "p2bht_rates_lfeq" + std::to_string(int(load_factor * 100)) + ".csv";
  bool output_file_exist = std::filesystem::exists(dir + fname);
  std::fstream output(dir + fname, std::ios::app);
...
int main(int argc, char** argv) {
  ...
  std::size_t min_keys = get_arg_value<uint64_t>(arguments, "min-keys")
                             .value_or(512000);  // 16384;//8192;//4096;
  std::size_t max_keys =
      get_arg_value<uint64_t>(arguments, "max-keys").value_or(min_keys * 10);
  std::size_t keys_step =
      get_arg_value<uint64_t>(arguments, "step").value_or((max_keys - min_keys) / 10);
  float load_factor1 = get_arg_value<float>(arguments, "load-factor1").value_or(0.8);
  float load_factor2 = get_arg_value<float>(arguments, "load-factor2").value_or(0.9);

  std::cout << "Device id = " << device_id << '\n';
  std::cout << "validate = " << std::boolalpha << validate << '\n';
  std::cout << "min_keys = " << min_keys << '\n';
  ...
  std::string dir = "./results/" + device_name + "/" + "rates_fixed_lf/";
  std::filesystem::create_directories(dir);

  std::vector<std::size_t> exp_num_keys;
  for (std::size_t k = min_keys; k <= max_keys; k *= keys_step) {
    exp_num_keys.push_back(k);
    std::cout << "keys=" << k << '\n';
  }
  std::vector<float> load_factors{load_factor2};
  for (auto& num_keys : exp_num_keys) {
