...
struct bench_insert_find_result {
  ...
  std::vector<float> find_probes;
  std::vector<float> insert_probes;
};
...
bench_insert_find_result compute_experiment_averages(
    ...
    const std::vector<float>& find_probes,
    const std::vector<float>& insert_probes) {
  float avg_insert_time = insert_time / float(num_experiments);

  std::vector<float> avg_find_times(find_times.size(), 0.0f);
  std::vector<float> avg_find_probes(find_times.size(), 0.0f);
  std::vector<float> avg_insert_probes(1, 0.0f);
  for (size_t i = 0; i < find_times.size(); i++) {
    avg_find_times[i] = find_times[i] / float(num_experiments);
    avg_find_probes[i] = find_probes[i] / float(num_experiments);
  }
  avg_insert_probes[0] = insert_probes[0] / float(num_experiments);

  return {avg_insert_time, avg_find_times, avg_find_probes, avg_insert_probes};
}
...
bench_insert_find_result bench_insert_find(
  ...
  std::cout << "Bench insert/search probes: " << num_keys << " keys, ";
  ...
  std::vector<float> find_probes(exist_ratios.size(), 0.0f);
  std::vector<float> insert_probes(1, 0.0f);
  int exp = 0;
  ...
  while (exp < num_experiments) {
    ...
    auto num_probes = bght::get_num_probes();
    if (!success) {
      failed_count++;
      if (failed_count == max_failed_count) {
        return {-1.0f,
                std::vector<float>(exist_ratios.size(), -1.0f),
                std::vector<float>(exist_ratios.size(), -1.0f),
    ...
    for (std::size_t ratio_idx = 0; ratio_idx < exist_ratios.size(); ratio_idx++) {
      ...
      find_probes[ratio_idx] += bght::get_num_probes();
      if (validate) {
  ...
  return compute_experiment_averages(
      num_experiments, insert_time, find_times, find_probes, insert_probes);
}
...
void bench_bcht(std::vector<key_type>& keys,
  ...
  std::string fname = "bcht_probes.csv";
  ...
  {
    auto bcht_8_result =
        bench_insert_find<bght::bcht8<key_type, value_type>>(keys,
                                                             d_keys,
                                                             d_pairs,
                                                             d_find_keys,
                                                             d_find_results,
                                                             ...
                                                             to_value,
                                                             8);
    output << bcht_8_result.insert_probes[0] / float(num_keys) << ",";
    for (size_t i = 0; i < bcht_8_result.find_probes.size(); i++) {
      output << bcht_8_result.find_probes[i] / float(num_keys) << ",";
    }
  }
  ...
  {
    auto bcht_16_result =
        bench_insert_find<bght::bcht16<key_type, value_type>>(keys,
                                                              d_keys,
                                                              d_pairs,
                                                              d_find_keys,
                                                              d_find_results,
                                                              ...
                                                              to_value,
                                                              16);
    output << bcht_16_result.insert_probes[0] / float(num_keys) << ",";
    for (size_t i = 0; i < bcht_16_result.find_probes.size(); i++) {
      output << bcht_16_result.find_probes[i] / float(num_keys) << ",";
    }
  }
  ...
  {
    auto bcht_32_result =
        bench_insert_find<bght::bcht32<key_type, value_type>>(keys,
                                                              d_keys,
                                                              d_pairs,
                                                              d_find_keys,
                                                              d_find_results,
                                                              ...
                                                              to_value,
                                                              32);
    output << bcht_32_result.insert_probes[0] / float(num_keys) << ",";
    for (size_t i = 0; i < bcht_32_result.find_probes.size(); i++) {
      output << bcht_32_result.find_probes[i] / float(num_keys) << ",";
    }
  }
  output << std::endl;
}
...
void bench_iht(std::vector<key_type>& keys,
  ...
  std::string fname = "iht_probes.csv";
  bool output_file_exist = std::filesystem::exists(dir + fname);
  std::fstream output(dir + fname, std::ios::app);
  ...
  {
    auto iht_16_result =
        bench_insert_find<bght::iht16<key_type, value_type, 3>>(keys,
                                                                d_keys,
                                                                d_pairs,
                                                                d_find_keys,
                                                                d_find_results,
                                                                ...
                                                                to_value,
                                                                16);
    output << iht_16_result.insert_probes[0] / float(num_keys) << ",";
    for (size_t i = 0; i < iht_16_result.find_probes.size(); i++) {
      output << iht_16_result.find_probes[i] / float(num_keys) << ",";
    }
  }
  ...
  {
    auto iht_16_result =
        bench_insert_find<bght::iht16<key_type, value_type, 6>>(keys,
                                                                d_keys,
                                                                d_pairs,
                                                                d_find_keys,
                                                                d_find_results,
  ...
  {
    auto iht_16_result =
        bench_insert_find<bght::iht16<key_type, value_type, 9>>(keys,
                                                                d_keys,
                                                                d_pairs,
                                                                d_find_keys,
                                                                d_find_results,
  ...
  {
    auto iht_16_result =
        bench_insert_find<bght::iht16<key_type, value_type, 12>>(keys,
                                                                 d_keys,
                                                                 d_pairs,
                                                                 d_find_keys,
                                                                 d_find_results,
  ...
  {
    auto iht_32_result =
        bench_insert_find<bght::iht32<key_type, value_type, 6>>(keys,
                                                                d_keys,
                                                                d_pairs,
                                                                d_find_keys,
                                                                d_find_results,
                                                                ...
                                                                to_value,
                                                                32);
    output << iht_32_result.insert_probes[0] / float(num_keys) << ",";
    for (size_t i = 0; i < iht_32_result.find_probes.size(); i++) {
      output << iht_32_result.find_probes[i] / float(num_keys) << ",";
    }
  }
  ...
  {
    auto iht_32_result =
        bench_insert_find<bght::iht32<key_type, value_type, 12>>(keys,
                                                                 d_keys,
                                                                 d_pairs,
                                                                 d_find_keys,
                                                                 d_find_results,
  ...
  {
    auto iht_32_result =
        bench_insert_find<bght::iht32<key_type, value_type, 19>>(keys,
                                                                 d_keys,
                                                                 d_pairs,
                                                                 d_find_keys,
                                                                 d_find_results,
  ...
  {
    auto iht_32_result =
        bench_insert_find<bght::iht32<key_type, value_type, 25>>(keys,
                                                                 d_keys,
                                                                 d_pairs,
                                                                 d_find_keys,
                                                                 d_find_results,
...
void bench_p2bht(std::vector<key_type>& keys,
  ...
  std::string fname = "p2bht_probes.csv";
  bool output_file_exist = std::filesystem::exists(dir + fname);
  std::fstream output(dir + fname, std::ios::app);
  ...
  // 16
  {
    auto p2cht_16_result =
        bench_insert_find<bght::p2bht16<key_type, value_type>>(keys,
                                                               d_keys,
                                                               d_pairs,
                                                               d_find_keys,
                                                               d_find_results,
                                                               ...
                                                               to_value,
                                                               16);
    output << p2cht_16_result.insert_probes[0] / float(num_keys) << ",";
    for (size_t i = 0; i < p2cht_16_result.find_probes.size(); i++) {
      output << p2cht_16_result.find_probes[i] / float(num_keys) << ",";
    }
  }
  ...
  // 32
  {
    auto p2cht_32_result =
        bench_insert_find<bght::p2bht32<key_type, value_type>>(keys,
                                                               d_keys,
                                                               d_pairs,
                                                               d_find_keys,
                                                               d_find_results,
                                                               ...
                                                               to_value,
                                                               16);
    output << p2cht_32_result.insert_probes[0] / float(num_keys) << ",";
    for (size_t i = 0; i < p2cht_32_result.find_probes.size(); i++) {
      output << p2cht_32_result.find_probes[i] / float(num_keys) << ",";
    }
  }
  output << std::endl;
}
...
int main(int argc, char** argv) {
  ...
  std::string dir = "./results/" + device_name + "/" + "avg_probes/";
