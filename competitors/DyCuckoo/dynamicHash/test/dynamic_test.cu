...
public:
    ...
    void batch_test() {
        ...
        std::vector<uint32_t> demanded = {261120, 522240, 1044480, 1199999, 2088960};
        dy_cuckoo.grow_ms.set_targets(demanded);
        dy_cuckoo.shrink_ms.set_targets(demanded);
        ...
        dy_cuckoo.grow_ms.dump_state("grow");
        dy_cuckoo.shrink_ms.dump_state("shrink");

        std::ofstream csv("dynamic_resize.csv");
        if(!csv.is_open()) {
            std::cerr << "Failed to open dynamic_resize.csv for writing\n";
            return;
        }
        csv << "num_buckets, grow_throughput, shrink_throughput\n";
        for (size_t i = 0; i < demanded.size(); i++) {
            csv << demanded[i] << ", "
                << dy_cuckoo.grow_ms.recorded[i] << ", "
                << dy_cuckoo.shrink_ms.recorded[i] << "\n";
        }
        csv.close();
...
int main(int argc, char** argv) {
    ...
    if (argc < 7)
    {
        std::cout << "Usage: ./dynamic_test <log2(pool size)> <r: ratio of deletions over insertions in a batch> <batch size> <lower bound> <upper bound> <init fill factor>\n";
        std::cout << "Example: ./dynamic_test 24 2 20 0.5 0.85 0.85\n";
        cout << "para error\n" << endl;
        return -1;
    }

    test_t dy_test;
    int pool_len = 1 << atoi(argv[1]);
    dy_test.pool_len = pool_len;
    dy_test.r = atoi(argv[2]);
    //batch size: 2e5 4e5 6e5 8e5 10e5
    dy_test.batch_size = 1 << atoi(argv[3]);
    dy_test.lower_bound = atof(argv[4]);
    dy_test.upper_bound = atof(argv[5]);
    dy_test.init_fill_factor = atof(argv[6]);
    test_t::key_t* keys_h = gen_unique_keys_u32<test_t::key_t>(1<<24, /*seed=*/42u + pool_len, /*shuffle=*/true);
