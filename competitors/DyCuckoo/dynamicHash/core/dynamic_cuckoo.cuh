...
#include "../include/Common/helper_functions.h"
#include "../include/Common/helper_cuda.h"
#include <stdint.h>
...
struct MilestoneAgg {
    ...
    uint64_t cum_buckets = 0;

    std::vector<uint32_t> targets;
    std::vector<double> recorded;  
    std::vector<char> done;

    void set_targets(const std::vector<uint32_t>& demanded) {
        targets = demanded;
        recorded.assign(demanded.size(), 0.0);
        done.assign(demanded.size(), 0);
        printf("[MilestoneAgg] targets set: ");
        for (auto t : targets) printf("%u ", t);
        printf("\n");
    }

    void add_event(double time_s, uint32_t delta_buckets) {
        // prove this is called
        if (targets.empty()) {
            printf("[MilestoneAgg] WARNING: add_event called but targets empty!\n");
            return;
        }

        cum_time_s += time_s;
        cum_buckets += delta_buckets;

        // avoid div0
        if (cum_time_s <= 0.0) {
            printf("[MilestoneAgg] WARNING: cum_time_s <= 0 (%.9f)\n", cum_time_s);
            return;
        }

        double thr = (double(cum_buckets) / cum_time_s) / 1e6; 

        // record when crossing
        for (size_t i = 0; i < targets.size(); i++) {
            if (done[i]) continue;

            if (cum_buckets >= (uint64_t)targets[i]) {
                recorded[i] = thr;
                done[i] = 1;
                printf("[MilestoneAgg] HIT target %u at cum_buckets=%llu, thr=%.3f\n",
                       targets[i], (unsigned long long)cum_buckets, thr);
            }
        }
    }

    void dump_state(const char* tag) const {
        //Dump for both grow and shrink
        
        printf("[MilestoneAgg:%s] cum_buckets=%llu cum_time_s=%.9f\n",
               tag, (unsigned long long)cum_buckets, cum_time_s);
        printf("[MilestoneAgg:%s] recorded: ", tag);
        
        for (double v : recorded) printf("%.3f ", v);
        printf("\n");
    }
};
...
public:
    ...
    MilestoneAgg grow_ms;
    MilestoneAgg shrink_ms;
    ...
    void resize_up(){
        ...
        uint32_t delta_buckets = new_size - old_size;
        grow_ms.add_event(diff, delta_buckets);
    ...
    void resize_down(){
        ...
        timer.Stop();
        double diff = timer.Elapsed();
        uint32_t delta_buckets = old_size - new_size;
        shrink_ms.add_event(diff, delta_buckets);
        cnmemFree(old_table, 0);
    }
