...
public:
    ...
    double hash_insert(key_t *keys, value_t* values, uint32_t size){
    ...
    double hash_search(key_t *keys, value_t *values, uint32_t size){
    ...
    double hash_delete(key_t *keys, value_t *values, uint32_t size){
        key_t *dev_keys;
        value_t* dev_values;
        cnmemMalloc((void**)&dev_keys, sizeof(key_t) * size, 0);
        cudaMemcpy(dev_keys, keys, sizeof(key_t) * size, cudaMemcpyHostToDevice);
