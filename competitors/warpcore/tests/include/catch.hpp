...
namespace Catch {
    ...
    // NVCC/libcu++: keep a fixed compile-time constant
    static constexpr std::size_t sigStackSize = 32768;
    #else
    // Host-only: runtime-aware size using sysconf (non-constexpr)
    static const std::size_t sigStackSize = []{
        long v = sysconf(_SC_SIGSTKSZ);
        std::size_t s = (v > 0) ? static_cast<std::size_t>(v) : 32768;
        return s < 32768 ? 32768 : s;
    }();
    #endif
    ...
    #else
    char FatalConditionHandler::altStackMem[Catch::sigStackSize] = {};
    #endif
